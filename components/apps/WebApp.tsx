...
export default function WebApp({ url, title }: Props) {
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-neutral-950 text-neutral-100">
      <div className="flex flex-shrink-0 items-center gap-2 border-b border-white/[0.06] bg-white/[0.02] px-3 py-2 text-xs">
        <button
          type="button"
          onClick={() => setReloadKey((k) => k + 1)}
          className="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-neutral-200 hover:bg-white/10"
          aria-label="Reload"
          title="Reload"
        >
        ...
        <a
          ...
          className={cn(
            "inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-white",
            accent,
          )}
        >
